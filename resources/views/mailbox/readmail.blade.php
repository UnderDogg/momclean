    ...
    <div class="box box-primary">
        ...
        <div class="box-body no-padding">
            <div class="mailbox-read-info">
                <h3>{!! $thread->title !!}</h3>
                <h5>From: {!! $thread->email !!} <span
                            class="mailbox-read-time pull-right">{!! $thread->created_at !!}</span></h5>
            </div>
                ...
                if ($attachment == null) {
                    ...
                    foreach ($attachments as $attachment) {
                        ...
                        if ($attachment->type == 'jpg' || $attachment->type == 'png') {
                            ...
                            $body = str_replace($attachment->name,
                                "data:image/" . $attachment->type . ";base64," . base64_encode($data), $body);
                    ...
                    foreach ($attachments as $attachment) {
                        ...
                        if ($attachment->poster == 'ATTACHMENT') {
                            if ($attachment->type == 'jpg' || $attachment->type == 'JPG' || $attachment->type == 'jpeg' || $attachment->type == 'JPEG' || $attachment->type == 'png' || $attachment->type == 'PNG' || $attachment->type == 'gif' || $attachment->type == 'GIF') {
                                ...
                                $data = ob_get_contents();
                                ob_end_clean();
                                $var = '<a href="' . URL::route('image',
                                        array('image_id' => $attachment->id)) . '" target="_blank"><img style="max-width:200px;height:133px;" src="data:image/jpg;base64,' . base64_encode($data) . '"/></a>';
                                ...
                                $var = '<a style="max-width:200px;height:133px;color:#666;" href="' . URL::route('image',
                                        array('image_id' => $attachment->id)) . '" target="_blank"><span class="mailbox-attachment-icon" style="background-color:#fff;">' . strtoupper($attachment->type) . '</span><div class="mailbox-attachment-info"><span ><b style="word-wrap: break-word;">' . $attachment->name . '</b><br/><p>' . $value . '</p></span></div></a>';
